...
import (
	...
	"github.com/jalapeno/bgpv4-graph/pkg/kafkanotifier"
...
func NewDBSrvClient(arangoSrv, user, pass, dbname, peer, ebgppeerV4, unicastprefixV4,
	...
	//glog.Infof("check ebgpv4 peer collection")
	...
	//glog.Infof("create ebgpv4 prefix collection")
	...
	//glog.Infof("check ebgpv4 prefix collection")
	...
	if found {
		c, err := arango.db.Graph(context.TODO(), bgpv4Graph)
		if err != nil {
			return nil, err
		}
		...
		glog.Infof("creating graph %s", bgpv4Graph)
		arango.bgpv4Graph, err = arango.db.CreateGraph(context.TODO(), bgpv4Graph, &options)
		if err != nil {
			return nil, err
		}
	}
	...
	arango.bgpv4Graph, err = arango.db.Graph(context.TODO(), bgpv4Graph)
	glog.Infof("checking collection %s", bgpv4Graph)
	if err != nil {
		return nil, err
	}
	...
	if arango.bgpv4Graph != nil {
		// Get the edge collection from the graph
		arango.graph, err = arango.db.Collection(context.TODO(), "bgpv4_graph")
		if err != nil {
		...
		return nil, fmt.Errorf("bgpv4Graph is nil")
	}
